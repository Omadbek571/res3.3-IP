...
import { AxiosError } from "axios"; // AxiosError importi kerak bo'lishi mumkin
import axiosInstance from "../../lib/axios"; // <<< Bizning axiosInstance ni import qilamiz
...
// const API_BASE_URL = "https://oshxonacopy.pythonanywhere.com/api"; // <<< Endi axiosInstance da baseURL mavjud
// Shuning uchun bu konstantadan bevosita foydalanilmaydi
...
const handleApiError = (error: any, contextMessage: string = "Xatolik") => {
  ...
  console.error(`${contextMessage} (API):`, error);
...
function POSPage() {
  ...
  const [isFetchingKitchenReceipt, setIsFetchingKitchenReceipt] =
    useState(false);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await axiosInstance.get("/categories/"); // <<< O'zgartirildi
      return res.data || [];
    },
    onError: (err: any) => {
      console.error("Kategoriyalarni yuklashda xato:", err);
      // handleApiError(err, "Kategoriyalarni yuklash");
    },
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await axiosInstance.get("/products/"); // <<< O'zgartirildi
      return res.data || [];
    },
    onError: (err: any) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await axiosInstance.get("/tables/"); // <<< O'zgartirildi
      return res.data || [];
    },
  ...
  const {
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const url = `/orders/${
        searchTerm ? `?search=${encodeURIComponent(searchTerm as string)}` : ""
      }`; // <<< O'zgartirildi (API_BASE_URL olib tashlandi)
      const res = await axiosInstance.get(url); // <<< O'zgartirildi (axios va headers olib tashlandi)
      return res.data || [];
    },
  ...
  const handlePrintKitchenReceiptViaApi = async (
    orderDataForKitchen: any,
    receiptType: "initial" | "delta_added" | "delta_cancelled" = "initial"
  ) => {
    if (!orderDataForKitchen || !orderDataForKitchen.id) {
      console.error("Oshxona cheki uchun buyurtma IDsi topilmadi.");
      setSubmitEditError("Oshxona cheki uchun buyurtma IDsi topilmadi.");
      return;
    }
    if (
      !Array.isArray(orderDataForKitchen.items) ||
      orderDataForKitchen.items.length === 0
    ) {
      console.warn(
        "Oshxona cheki uchun mahsulotlar yo'q. Faqat buyurtma ochilgani haqida xabar yuborilishi mumkin."
      );
    }
    ...
    const payload = {
      /* ... payload ... */
    }; // Payload o'zgarishsiz qoladi

    if (payload.items.length === 0 && receiptType === "initial") {
      payload.items = [
        { productName: "YANGI BUYURTMA (BO'SH)", quantity: 1, reason: null },
      ];
    }
    ...
    try {
      ...
      // });
      const response = await axiosInstance.post("/print/kitchen/", payload); // <<< O'zgartirildi
      if (response.data && response.data.success) {
        console.log(
          response.data.message ||
            `Oshxona cheki (${receiptType}) chop etishga yuborildi.`
        );
      } else {
        console.error(
          response.data.message ||
            "Oshxona chekini chop etishda noma'lum xatolik (API)."
        );
        setSubmitEditError(
          response.data.message ||
            "Oshxona chekini chop etishda noma'lum xatolik (API)."
  ...
  const handlePrintReceiptViaApi = async (orderDataForReceipt: any) => {
    ...
    if (
      !Array.isArray(orderDataForReceipt.items) ||
      orderDataForReceipt.items.length === 0
    ) {
    ...
    try {
      ...
      //     headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }
      // });
      const response = await axiosInstance.post("/print/customer/", payload); // <<< O'zgartirildi
      if (response.data && response.data.success) {
        console.log(
          response.data.message || "Mijoz cheki chop etishga yuborildi."
        );
      } else {
        console.error(
          response.data.message ||
            "Mijoz chekini chop etishda noma'lum xatolik (API)."
        );
        setSubmitEditError(
          response.data.message ||
            "Mijoz chekini chop etishda noma'lum xatolik (API)."
  ...
  const finishEditingInternal = (informUser: boolean = false) => {
    ...
    if (informUser && previousId) {
      console.log(
        `Buyurtma #${previousId} bilan ishlash yakunlandi/bekor qilindi.`
      );
    }
  };

  const loadOrderForEditing = async (
    orderIdToLoad: number,
    associatedTable: any = null
  ) => {
    const token = getToken(); // Bu tekshiruv shu yerda qolishi mumkin
    ...
    try {
      const data = await queryClient.fetchQuery<any>({
        ...
        queryFn: async () => {
          ...
          // const res = await axios.get(url, { headers: { Authorization: `Bearer ${token}` } });
          // Yuqoridagi 'token' shu queryFn uchun ham kerak, lekin axiosInstance o'zi qo'shadi.
          // Agar queryFn ichida alohida token tekshiruvi kerak bo'lsa:
          const currentTokenInQuery = getToken();
          if (!currentTokenInQuery)
            throw new Error("fetchQuery uchun token topilmadi");

          const res = await axiosInstance.get(`/orders/${orderIdToLoad}/`); // <<< O'zgartirildi
          if (!res.data)
            throw new Error(
              `Buyurtma (#${orderIdToLoad}) ma'lumoti topilmadi.`
            );
          return res.data;
        },
        staleTime: 0,
      ...
      if (
        ...
        console.warn(
          `Buyurtma #${orderIdToLoad} (${data.status_display}) holatida tahrirlab bo'lmaydi.`
        );
        setSubmitEditError(
          `Buyurtma #${orderIdToLoad} (${data.status_display}) tahrirlanmaydi.`
        );
        setIsEditLoadingManual(false);
  ...
  const createOrderMutation = useMutation({
    mutationFn: async (orderData: any) => {
      ...
      if (dataToSend.customer_phone) {
        dataToSend.customer_phone = dataToSend.customer_phone.replace(
          /\D/g,
          ""
        );
      }
      // const res = await axios.post(`${API_BASE_URL}/orders/`, dataToSend, { headers: { Authorization: `Bearer ${token}` } });
      const res = await axiosInstance.post("/orders/", dataToSend); // <<< O'zgartirildi
      return res.data;
    },
    onSuccess: (data) => {
      // ... onSuccess logikasi o'zgarishsiz ...
    ...
    onError: (error: any, variables: any) => {
      ...
      if (error.response?.data) {
        ...
        if (typeof errorData === "string") msg = errorData;
          ...
          errorData.table_id &&
          Array.isArray(errorData.table_id) &&
          errorData.table_id[0]?.includes("is already occupied")
        ) {
          queryClient.invalidateQueries({ queryKey: ["tables"] });
          const tableNameFromState = tables.find(
            (t: any) => t.id === variables.table_id
          )?.name;
          msg = `Stol ${
            tableNameFromState || variables.table_id || "Tanlangan stol"
          } hozirda band.`;
  ...
  const updateOrderItemsMutation = useMutation({
    mutationFn: async ({
      ...
      const url = `/orders/${orderId}/update-items/`; // <<< O'zgartirildi
      const res = await axiosInstance.post(url, payload); // <<< O'zgartirildi
      return res.data;
    },
    onMutate: () => {
      setSubmitEditError(null);
    ...
    onSuccess: (data, variables) => {
      ...
      newItemsFromResponse.forEach((newItem: any) => {
        const oldItem = originalOrderItems.find(
          (oi) =>
            (oi.product_id || oi.product) ===
            (newItem.product_id || newItem.product)
        );
        if (!oldItem) {
          addedOrIncreasedItems.push({
            ...newItem,
            productName: newItem.product_details?.name,
      ...
      originalOrderItems.forEach((oldItem: any) => {
        const newItem = newItemsFromResponse.find(
          (ni) =>
            (ni.product_id || ni.product) ===
            (oldItem.product_id || oldItem.product)
        );
        if (!newItem) {
          removedOrDecreasedItems.push({
            ...oldItem,
            productName: oldItem.product_details?.name,
      ...
      queryClient.setQueryData(
        ["orderDetails", variables.orderId],
        updatedOrderFromServer
      );
      queryClient.invalidateQueries({ queryKey: ["tables"] });
      if (showHistoryDialog) {
    ...
    onError: (error: any) => {
      ...
      if (error.response?.data) {
        ...
        errorMsg = `Ulanish xatosi: ${error.message}`;
      }
  ...
  const checkoutMutation = useMutation({
    mutationFn: async ({
      ...
      const url = `/tables/${tableId}/checkout/`; // <<< O'zgartirildi
      const res = await axiosInstance.post(url, paymentData); // <<< O'zgartirildi
      return res.data;
    },
    onSuccess: (data, variables) => {
      // ... onSuccess logikasi o'zgarishsiz ...
      console.log(
        `Stol #${variables.tableId} uchun to'lov amalga oshirildi! Buyurtma #${data.id} yopildi.`
      );
      if (data && Array.isArray(data.items) && data.items.length > 0) {
        handlePrintReceiptViaApi(data);
      }
    ...
    onError: (error: any, variables) => {
      ...
      setSubmitEditError(`Xatolik: ${msg}`);
      console.error(
        `To'lovda xato (Stol #${variables.tableId}): ${msg}`,
        error
      );
      if (
        error.response?.status === 404 ||
        error.response?.data?.detail?.includes("active order")
      ) {
        queryClient.invalidateQueries({ queryKey: ["tables"] });
      }
    },
  ...
  const reorderMutation = useMutation({
    mutationFn: async (orderData: any) => {
      ...
      const { originalOrderId, originalOrderData, ...actualOrderData } =
        dataToSend;
      // const res = await axios.post(`${API_BASE_URL}/orders/`, actualOrderData, { headers: { Authorization: `Bearer ${token}` } });
      const res = await axiosInstance.post("/orders/", actualOrderData); // <<< O'zgartirildi
    ...
    onSuccess: (response, variables: any) => {
      ...
      console.log(
        `Buyurtma #${originalOrderId} dan nusxa (#${newData.id}) yaratildi!`
      );
    ...
    onError: (error: any, variables: any) => {
      ...
      if (error.response?.data) {
        ...
        if (typeof errorData === "string") msg = errorData;
          ...
          const tableNameFromState = tables.find(
            (t: any) => t.id === originalOrder?.table?.id
          )?.name;
          const tableName =
            tableNameFromState ||
            originalOrder?.table_name ||
            originalOrder?.table?.id ||
            "Stol";
  ...
  const currentPanelTotal = useMemo(() => {
    let itemsTotal = 0;
    if (editingOrderId && orderToEdit?.items) {
      itemsTotal = orderToEdit.items.reduce(
        (sum: number, item: any) =>
          sum + Number(item.unit_price || 0) * item.quantity,
        0
      );
    } else if (!editingOrderId && cart.length > 0) {
      itemsTotal = cart.reduce(
        (total: number, cartItem: any) =>
          total + parseFloat(cartItem.product?.price || 0) * cartItem.quantity,
        0
      );
    }
    return itemsTotal;
  ...
  // ... Bu qismlardagi logikalar API chaqiruvlariga bog'liq emas, shuning uchun o'zgarishsiz qoladi ...
  ...
  const handleLocalEditQuantityChange = (productId: number, change: number) => {
    if (!editingOrderId || !orderToEdit || updateOrderItemsMutation.isPending)
      return;
    setOrderToEdit((prevOrder: any) => {
      if (!prevOrder) return null;
      const updatedItems = prevOrder.items ? [...prevOrder.items] : [];
      const itemIndex = updatedItems.findIndex(
        (item) => (item.product_id || item.product) === productId
      );
      if (itemIndex > -1) {
  ...
  const handleLocalAddItemFromProductList = (product: any) => {
    ...
    setOrderToEdit((prevOrder: any) => {
      ...
      const updatedItems = prevOrder.items ? [...prevOrder.items] : [];
      const itemIndex = updatedItems.findIndex(
        (item) => (item.product_id || item.product) === product.id
      );
      if (itemIndex > -1) {
  ...
  const submitOrder = () => {
    ...
    if (
      ...
      (!customerInfo.name || phoneDigits.length < 12)
    ) {
  ...
  const submitEditedOrderChanges = () => {
    ...
    if (
      orderToEdit.status === "paid" ||
      orderToEdit.status === "completed" ||
      orderToEdit.status === "cancelled"
    ) {
      console.error(
        `Buyurtma #${editingOrderId} (${orderToEdit.status_display}) holatida, o'zgartirib bo'lmaydi.`
      );
      setSubmitEditError(
        `Buyurtma #${editingOrderId} (${orderToEdit.status_display}) holatida, o'zgartirib bo'lmaydi.`
      );
    ...
    currentItems.forEach((currentItem: any) => {
      const originalItem = originalOrderItems.find(
        (o) =>
          (o.product_id || o.product) ===
          (currentItem.product_id || currentItem.product)
      );
      if (!originalItem) {
        operations.push({
          operation: "add",
          product_id: currentItem.product_id || currentItem.product,
        ...
        if (originalItem.id && typeof originalItem.id === "number") {
          ...
          console.warn(
            "Set operatsiyasi uchun order_item_id topilmadi, product_id ishlatiladi:",
            originalItem
          );
          operations.push({
            operation: "set_by_product",
            product_id: originalItem.product_id || originalItem.product,
            quantity: currentItem.quantity,
          });
        }
      }
      ...
      if (
        !currentItems.find(
          (c: any) =>
            ...
            (originalItem.product_id || originalItem.product)
        )
      ) {
        if (originalItem.id && typeof originalItem.id === "number") {
          operations.push({
          ...
          console.warn(
            "Remove operatsiyasi uchun order_item_id topilmadi:",
            originalItem
          );
          operations.push({
            operation: "remove_by_product",
            product_id: originalItem.product_id || originalItem.product,
          });
        }
      }
    ...
    updateOrderItemsMutation.mutate({
      ...
      payload: { items_operations: operations },
    });
  };
  ...
  const reorderToSameTable = (order: any) => {
    ...
    if (order.status !== "completed" && order.status !== "paid") {
      console.warn(
        "Bu funksiya faqat yakunlangan yoki to'langan buyurtmalar uchun."
      );
      return;
    }
    const tableIdForReorder =
      order.order_type === "dine_in" ? order.table?.id || order.table_id : null;
    if (order.order_type === "dine_in" && !tableIdForReorder) {
      console.error("Qayta buyurtma berish uchun stol ma'lumotlari topilmadi.");
      setSubmitEditError(
        "Qayta buyurtma berish uchun stol ma'lumotlari topilmadi."
      );
      return;
    }
    const orderData = {
      ...
      customer_phone:
        order.order_type === "takeaway" || order.order_type === "delivery"
          ...
          : null,
      customer_address:
        order.order_type === "delivery" ? order.customer_address : null,
  ...
  const handleCustomerInfoSave = () => {
    ...
    if (!customerInfo.name || phoneDigits.length < 12) {
      console.error(
        "Ism va telefon raqamini to'liq kiriting (kamida 12 raqam)."
      );
    ...
    setShowCustomerDialog(false);
    console.log("Mijoz ma'lumotlari saqlandi.");
    setSubmitEditError(null);
  };
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCustomerInfo((prev) => ({
      ...prev,
      phone: formattedNumber.slice(0, 17),
    }));
  };
  ...
  const cancelEditing = () => {
    ...
    console.info("Tahrirlash bekor qilindi.");
  };
  ...
  const confirmLogoutAction = () => {
    if (typeof window !== "undefined") {
      ...
      localStorage.removeItem("refresh"); // Refresh tokenni ham o'chirish
      queryClient.clear();
  ...
  // === JSX ===
  return (
    // ... JSX qismi asosan o'zgarishsiz qoladi, faqat yuqoridagi logikaga bog'liq joylari
    // yangilangan API chaqiruvlari bilan ishlashi kerak.
    // Masalan, `isLoading*` va `error*` state'lari to'g'ri ishlashi kerak.
    <TooltipProvider>
      <div className="flex h-screen flex-col bg-muted/40">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-background px-4 sm:px-6 shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <h1 className="text-lg sm:text-xl font-bold hidden md:inline-block">
              SmartResto POS
            </h1>
          </div>
          <div className="flex-1 flex justify-center px-4">
            <Tabs
              ...
              onValueChange={(value) => {
                if (editingOrderId || isMutationLoading) {
                  console.info(
                    "Joriy buyurtma bilan ishlash tugallanmaguncha turni o'zgartirib bo'lmaydi."
                  );
                  return;
                }
                if (orderType !== value) {
              ...
              className={`w-full max-w-md ${
                editingOrderId || isMutationLoading
            ...
            >
              <TabsList className="grid w-full grid-cols-3 h-11">
                <TabsTrigger
                  value="dine_in"
                  disabled={!!editingOrderId || isMutationLoading}
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-0 overflow-hidden">
          ...
          <div className="md:col-span-1 lg:col-span-2 flex flex-col bg-background overflow-hidden">
                ...
                {editingOrderId && orderToEdit ? (
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          className="h-8 w-8"
                          onClick={() =>
                            handlePrintKitchenReceiptViaApi(
                              orderToEdit,
                              "initial"
                            )
                          }
                          disabled={
                            isFetchingKitchenReceipt ||
                            !orderToEdit.items ||
                            orderToEdit.items.length === 0 ||
                            isAnyLoading
                          }
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          disabled={
                            isAnyLoading || updateOrderItemsMutation.isPending
                          }
                        >
                  ...
                  <>
                    {orderType === "dine_in" && (
                      <>
                        {selectedTableId &&
                          tables.find((t: any) => t.id === selectedTableId) && (
                            <Badge
                              variant="outline"
                      ...
                      (customerInfo.name ? (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Badge
                              variant="secondary"
                              className="cursor-pointer text-xs px-1.5 py-0.5 h-10 flex items-center"
                              onClick={() => {
                                setSubmitEditError(null);
                                setShowCustomerDialog(true);
                              }}
            ...
            <ScrollArea className="flex-1 p-4">
              {isEditLoadingManual ? (
                <div className="flex h-full items-center justify-center">
                  <Loader2 className="h-6 w-6 animate-spin mr-2" /> Buyurtma
                  yuklanmoqda...
                </div>
              ) : editErrorManual ? (
                <div className="text-destructive p-4 text-center">
                  ...
                  <Button
                    variant="link"
                    onClick={() =>
                      editingOrderId && loadOrderForEditing(editingOrderId)
                    }
                  >
                    Qayta urinish
                  </Button>
                ...
                <div className="text-muted-foreground text-center p-10">
                  ...
                  {editingOrderId ? "Buyurtmada mahsulot yo'q" : "Savat bo'sh"}
                </div>
              ) : (
                <div className="space-y-3">
                  {currentPanelItems.map((item: any, index: number) => {
                    const productInfo = editingOrderId
                    ...
                    const productImage = editingOrderId
                      ? productInfo?.image_url
                      : productInfo?.image;
                    const unitPrice = editingOrderId
                      ? item.unit_price
                      : productInfo?.price;
                    const itemKey = editingOrderId
                      ? item.id ||
                        `temp-${item.product_id || item.product}-${index}`
                      : item.id;

                    return (
                      <div
                        key={itemKey}
                        className={`flex items-center justify-between space-x-2 border-b pb-3 last:border-b-0 ${
                          isAnyLoading &&
                          editingOrderId &&
                          updateOrderItemsMutation.isPending
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {Number(unitPrice || 0).toLocaleString("uz-UZ")}{" "}
                              so‘m
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="text-right shrink-0 w-24">
                          <p className="font-semibold text-sm">
                            {(
                              Number(unitPrice || 0) * item.quantity
              ...
              {submitEditError && !editErrorManual && (
                <p className="text-center text-destructive text-xs mt-4 p-2 bg-destructive/10 rounded">
                  {submitEditError}
                </p>
              )}
            </ScrollArea>
              ...
              <div className="space-y-1 mb-4 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {currentPanelTotal.toLocaleString("uz-UZ")} so‘m
                  </span>
                </div>
                {editingOrderId && orderToEdit && (
                  <>
                    ...
                    {Number(orderToEdit.service_fee_percent || 0) > 0 && (
                      <div className="flex justify-between text-xs">
                        <span className="text-muted-foreground">
                          Xizmat ({orderToEdit.service_fee_percent}%):
                        </span>
                        <span>
                          {Number(
                            orderToEdit.service_fee_amount ||
                              (currentPanelTotal *
                                Number(orderToEdit.service_fee_percent)) /
                                100
                          ).toLocaleString("uz-UZ")}{" "}
                          so'm
                        </span>
                    ...
                    <div className="flex justify-between font-semibold border-t pt-1 mt-1">
                      ...
                      <span>
                        {Number(
                          orderToEdit.final_price || currentPanelTotal
                        ).toLocaleString("uz-UZ")}{" "}
                        so‘m
                      </span>
                    </div>
              ...
              {editingOrderId && orderToEdit ? (
                <div className="space-y-2">
                  <Button
                    ...
                    disabled={
                      ...
                      currentPanelItems.length === 0 ||
                      !!editErrorManual ||
                      ["paid", "completed", "cancelled"].includes(
                        orderToEdit.status
                      )
                    }
                  >
                  ...
                  {orderToEdit.order_type === "dine_in" &&
                    orderToEdit.table &&
                    tables.find((t: any) => t.id === orderToEdit.table.id)
                      ?.active_order_id === editingOrderId &&
                    !["paid", "completed", "cancelled"].includes(
                      orderToEdit.status
                    ) && (
                      <Button
                        ...
                        onClick={() => {
                          const currentTable = tables.find(
                            (t: any) => t.id === orderToEdit.table.id
                          );
                        ...
                        disabled={isAnyLoading}
                      >
                        <CheckCircle className="mr-2 h-4 w-4" /> To'lov (
                        {Number(orderToEdit.final_price || 0).toLocaleString(
                          "uz-UZ"
                        )}{" "}
                        so‘m)
                      </Button>
                ...
                <Button
                  ...
                  disabled={
                    ...
                    ((orderType === "takeaway" || orderType === "delivery") &&
                      (!customerInfo.name ||
                        customerInfo.phone.replace(/\D/g, "").length < 12)) ||
                ...
                >
                  ...
                  Buyurtma ({currentPanelTotal.toLocaleString("uz-UZ")} so‘m)
                </Button>
              )}
            </div>
          </div>
        ...
        <Dialog open={showTableDialog} onOpenChange={setShowTableDialog}>
          <DialogContent className="sm:max-w-lg md:max-w-2xl lg:max-w-4xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              ...
              <DialogDescription>
                Buyurtma uchun stol tanlang yoki band stolni oching.
              </DialogDescription>
            </DialogHeader>
            <div className="my-4 flex items-center gap-4 px-1 sm:px-6">
              <Label htmlFor="zone-filter" className="shrink-0">
                Zona:
              </Label>
              <Select
              ...
              >
                <SelectTrigger id="zone-filter" className="w-full sm:w-[200px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              {isLoadingTables && !tables.length ? (
                ...
                <div className="text-destructive p-4 text-center">
                  ...
                  (t: any) =>
                    ...
                    (t.zone || "N/A") === selectedZoneFilter
                ).length ? (
                <p className="text-center text-muted-foreground py-10">
                  Bu zonada stol topilmadi.
                </p>
              ) : (
                      ...
                      .map((table: any) => (
                        ...
                        >
                          <Button
                            ...
                            ${
                              !table.is_available
                                ? "bg-red-100 border-red-400 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:border-red-700 dark:text-red-300 dark:hover:bg-red-800/40"
                            ...
                            onClick={() => {
                              ...
                              if (!table.is_available) {
                                if (table.active_order_id) {
                                  if (
                                    editingOrderId === table.active_order_id
                                  ) {
                                    setShowTableDialog(false);
                                    return;
                                  }
                                  ...
                                  console.warn(
                                    `Stol ${table.name} band, lekin aktiv buyurtmasi topilmadi. Ma'lumotlar yangilanmoqda...`
                                  );
                                ...
                                if (editingOrderId) {
                                  console.info(
                                    "Avval joriy buyurtmani saqlang yoki bekor qiling."
                                  );
                                  return;
                                }
                                if (orderType !== "dine_in") {
                                  ...
                                  setCart([]);
                                  console.info(
                                    "Buyurtma turi 'Shu yerda' ga o'zgartirildi."
                                  );
                                }
                          ...
                          >
                            ...
                            {!table.is_available && table.active_order_id && (
                              <div className="text-[10px] mt-0.5 text-blue-600 dark:text-blue-400">
                                ...
                                {parseFloat(
                                  table.active_order_final_price || "0"
                                ) > 0 && (
                                  <span>
                                    {parseFloat(
                                      table.active_order_final_price
                                    ).toLocaleString("uz-UZ")}{" "}
                                    so'm
                                  </span>
                                )}
                          ...
                          {!table.is_available &&
                            ...
                            !checkoutMutation.isPending && (
                              <Button
                                variant="destructive"
                                size="xs"
                                className="w-full text-[10px] px-1 py-0.5 h-auto"
                                onClick={() => {
                                  setTableForCheckout(table);
                                  setSubmitEditError(null);
                                  setShowCheckoutDialog(true);
                                }}
                                disabled={isAnyLoading}
            ...
            <DialogFooter className="px-1 sm:px-6 pb-6 pt-3 border-t">
              <DialogClose asChild>
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            if (!open) setSubmitEditError(null);
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="grid gap-4 py-4">
              <div className="space-y-1">
                <Label htmlFor="cust-name">Ism*</Label>
                <Input
                  id="cust-name"
              ...
              {orderType === "delivery" && (
                <div className="space-y-1">
                  <Label htmlFor="cust-addr">Manzil*</Label>
                  <Input
                    id="cust-addr"
            ...
            {submitEditError && (
              <p className="text-sm text-destructive text-center -mt-2 mb-2">
                {submitEditError}
              </p>
            )}
            <DialogFooter>
        ...
        <Dialog open={showLogoutDialog} onOpenChange={setShowLogoutDialog}>
          <DialogContent className="sm:max-w-[400px]">
            <DialogHeader>
              ...
              <DialogDescription>Tizimdan chiqmoqchimisiz?</DialogDescription>
            </DialogHeader>
            <DialogFooter className="mt-4">
              <DialogClose asChild>
                <Button variant="outline">Yo'q</Button>
              </DialogClose>
              <Button variant="destructive" onClick={confirmLogoutAction}>
                Ha, Chiqish
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={showHistoryDialog} onOpenChange={setShowHistoryDialog}>
          <DialogContent className="sm:max-w-xl md:max-w-3xl lg:max-w-5xl xl:max-w-7xl h-[90vh] flex flex-col">
            <DialogHeader>
              ...
              <DialogDescription>
                Tahrirlash uchun ustiga bosing (yakunlangan/bekor qilinganlarni
                tahrirlab bo'lmaydi).
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex-1 overflow-hidden px-1">
              <ScrollArea className="h-full px-2 sm:px-5 pb-6">
                {isHistoryLoading ? (
                  <div className="flex h-full items-center justify-center">
                  ...
                  <div className="text-destructive p-4 text-center">
                    Tarix yuklashda xatolik.
                  </div>
                ) : orderHistory.length === 0 ? (
                  <div className="text-muted-foreground text-center p-10">
                    {historySearchQuery
                      ? `"${historySearchQuery}" uchun topilmadi.`
                      : "Tarix bo'sh."}
                  </div>
                ) : (
                  <div className="space-y-4">
                    {[...orderHistory]
                      ...
                      .map((order: any) => (
                        <Card
                          key={order.id}
                          className={`overflow-hidden shadow-sm hover:shadow-md group relative ${
                          ...
                          onClick={() => {
                            if (
                              ...
                              console.info(
                                `Buyurtma #${order.id} (${order.status_display}) yakunlangan, tahrirlab bo'lmaydi.`
                              );
                              return;
                            }
                            if (isAnyLoading) {
                              console.info(
                                "Boshqa amal bajarilmoqda, iltimos kuting."
                              );
                              return;
                            }
                            ...
                            <div className="sm:col-span-6 md:col-span-2 space-y-1 text-right sm:text-left md:text-right flex flex-col items-end sm:items-start md:items-end justify-between">
                              ...
                              {(order.status === "completed" ||
                                ...
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="mt-2 text-xs h-7 px-2 py-1 self-end"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                ...
                                >
                                  {reorderMutation.isPending &&
                                  reorderMutation.variables?.originalOrderId ===
                                    order.id ? (
            ...
            <DialogFooter className="px-1 sm:px-6 py-3 border-t">
              <DialogClose asChild>
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                To'lov: Stol {tableForCheckout?.name}{" "}
                {tableForCheckout?.zone && tableForCheckout.zone !== "N/A"
                  ? `(${tableForCheckout.zone})`
                  : ""}
              </DialogTitle>
              <DialogDescription>
                Buyurtma #{tableForCheckout?.active_order_id} | Jami:{" "}
                <span className="font-semibold text-lg ml-1">
                  {parseFloat(
                    tableForCheckout?.active_order_final_price || "0"
                  ).toLocaleString("uz-UZ")}{" "}
                  so'm
                </span>
              </DialogDescription>
            ...
            <div className="py-4 space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setPaymentDetails((p) => ({
                        ...p,
                        received_amount: e.target.value.replace(/\D/g, ""),
                      }))
                    }
                    min={parseFloat(
                      tableForCheckout?.active_order_final_price || "0"
                  ...
                  />
                  {parseFloat(paymentDetails.received_amount) >=
                    parseFloat(
                      tableForCheckout?.active_order_final_price || "0"
                    ) && (
              ...
              {paymentDetails.method === "mobile" && (
                <div className="space-y-1">
                  ...
                  <Select
                    value={paymentDetails.mobile_provider}
                    onValueChange={(val) =>
                      setPaymentDetails((p) => ({ ...p, mobile_provider: val }))
                    }
                  >
                    <SelectTrigger id="mobile_provider">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Click">Click</SelectItem>
                      <SelectItem value="Payme">Payme</SelectItem>
                      <SelectItem value="UzPay">UzPay</SelectItem>
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => {
                  ...
                  if (paymentDetails.method === "cash") {
                    const receivedAmount = parseFloat(
                      paymentDetails.received_amount
                    );
                    if (isNaN(receivedAmount) || receivedAmount < finalPrice) {
                      console.error(
                        "Qabul qilingan naqd summa xato yoki yetarli emas."
                      );
                      setSubmitEditError(
                        "Qabul qilingan naqd summa xato yoki yetarli emas."
                      );
                  ...
                  if (paymentDetails.method === "mobile") {
                    if (!paymentDetails.mobile_provider) {
                      console.error(
                        "Mobil to'lov uchun provayder tanlanmagan."
                      );
                ...
                disabled={
                  checkoutMutation.isPending ||
                  (paymentDetails.method === "cash" &&
                    (isNaN(parseFloat(paymentDetails.received_amount)) ||
                      parseFloat(paymentDetails.received_amount) <
                        parseFloat(
                          tableForCheckout?.active_order_final_price || "0"
                        ))) ||
                  (paymentDetails.method === "mobile" &&
                    !paymentDetails.mobile_provider)
                }
              >
                {checkoutMutation.isPending ? (
